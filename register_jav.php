            ...
            <div class="container">
                ...
                <div class="form_box">
                    ...
                    <label class="side_label">
                        <input type="radio" class="user-type" name="destination" value="1" checked>会社
                    </label>
                    <label class="side_label">
                        <input type="radio" class="user-type" name="destination" value="2">学校
                    </label>
                ...
                <div class="company_form">
                    ...
                    <div class="form_box">
                        <div class="form_headline user-type-company" style="margin-top: 20px;">
                            団体名
                        </div>
                        <input type="text" class="<?= $organization_err ? 'input_error' : '' ?> type-company-placeholder" value="<?= $organization ?>" name="organization" placeholder="株式会社〇〇">
                        <?= $organization_err ? "<span class='error'>$organization_err</span>" : '' ?>
                    </div>

                    <div class="form_box">
                        <div class="form_headline user-type-school">
                            所属（引渡し先）
                        </div>
                        <input type="text" id="department" class="<?= $affiliates_err ? 'input_error' : '' ?> type-school-placeholder" name="affiliates" value="<?= $affiliates ?>" placeholder="営業部">
...
<script>
    ...
    userType.forEach(function(el) { 
        el.addEventListener('click', function() {
            let type = this.checked ? this.value : ''

            // Company
            document.querySelector('.type-company-placeholder').setAttribute('placeholder', type == 1 ? '株式会社〇〇' : '〇〇中学校')
            // School
            document.querySelector('.user-type-school').innerHTML = type == 1 ? '所属（引渡し先)' : '所属（学年・クラス)'
            document.querySelector('.type-school-placeholder').setAttribute('placeholder', type == 1 ? '営業部' : '〇年〇組')
        })
    })
